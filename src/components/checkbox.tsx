...
const Checkbox = forwardRef<HTMLInputElement, CheckboxProps>((props, ref) => {
  const { children, error, inputClassName, inputWidth = 16, inputHeight = 16, ...rest } = props;
    ...
    <label className={twMerge("flex flex-col text-sm", props.className)}>
      <div className="flex items-center relative">
        <input
          ...
          className={twMerge(
            clsx(
              "appearance-none border border-gray-400 rounded-none",
              "checked:bg-primary-400 checked:ring-primary-400",
              props.appearance === "circle" && ["appearance-none rounded-full", "checked:ring-1"],
              props.appearance === "square" && ["rounded-sm"],
              props.tick && [
                "checked:relative",
